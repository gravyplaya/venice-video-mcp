...
- Wan 2.7 i2v has **no** `reference_image_urls` --- its single `image_url` is the only identity anchor, so a panel-derived keyframe will drift mid-clip. See A27 for the fix.
...
### A27. Wan 2.7 dialogue clips drift on identity --- keyframe from Seedance R2V, not from a panel
...
**Cause:** Wan 2.7 i2v exposes no `reference_image_urls`. Its only identity anchor is the single `image_url` keyframe. The harness defaults that keyframe to the shot's PANEL, but panels are generated by image models (`seedream-v5-lite` / `nano-banana-pro`) without strong 4-angle character conditioning, so Wan has nothing strong to lock onto.
**Fix (harness CLAUDE.md rule 32 / validated 2026-05-12):** Two-stage render for the affected shot. The harness does not yet automate this in `media.generate_videos` --- it's a manual workflow for now. For each character-dialogue shot where identity matters:

1. **Force the shot to Seedance R2V first.** In `script.json` for the shot, set `motion: 'high'` (so the planner skips Wan 2.7 routing) and remove or null out `dialogue` temporarily, then `media.generate_videos { project, episode }`. This renders the shot with all four character refs anchored.
2. **Extract the first frame as the keyframe.**
   ```bash
   ffmpeg -i <episodeDir>/scene-001/shot-NNN.mp4 -frames:v 1 -update 1 \
     <episodeDir>/scene-001/shot-NNN-panel.png
   ```
   This overwrites the panel with the identity-locked first frame (archive the prior panel first per A18 if the user wants to keep it).
3. **Restore the dialogue and re-render.** Reset `motion` to `low` or `medium`, restore the `dialogue` block in `script.json`, then `media.generate_videos` again --- the planner now routes to Wan 2.7 i2v, which keyframes off the locked-identity panel and synthesizes lip-sync to the dialogue audio.

**Cost:** ~$0.85 per shot total (Seedance R2V + Wan 2.7 i2v --- roughly 2× the single-pass Wan price).
**Apply when:** Character is on-screen, dialogue exists, `motion` is `low | medium`.
**Skip when:**
- No dialogue --- just use Seedance R2V alone (single-pass).
- `motion: 'high'` --- Wan drifts regardless of keyframe, use Seedance R2V and accept no lip-sync.
- Multi-character dialogue with different speakers --- use `wan-2-7-reference-to-video` (R2V variant supporting `per_reference_audio` per element) instead.

When the harness lands an automatic 2-stage path for this, this anti-pattern goes away --- track the harness release notes via `inspect.models` / the MCP's update check.
